...
function useShouldDisableAnimations(disableAnimations: boolean) {
  ...
  useEffect(() => {
    ...
    const sync = () => {
      setAutoDisable(
        motionQuery.matches || window.innerWidth <= MOBILE_BREAKPOINT,
      );
    };
    sync();
  ...
  return disableAnimations || autoDisable;
}
...
/** Single surface with MagicBento glow / particles — wraps existing auth frames */
export function MagicBentoCard({
  ...
  const magnetismAnimationRef = useRef<ReturnType<typeof gsap.to> | null>(
    null,
  );

  const shouldDisable = useShouldDisableAnimations(disableAnimations);
  ...
  const classes = [
    "magic-bento-card",
    enableStars ? "magic-bento-particle-host" : "",
...
/** Shared cursor spotlight across MagicBento cards in a section */
export function MagicBentoSpotlight({
  ...
  useEffect(() => {
    if (shouldDisable || !sectionRef.current || !enabled) return;
  ...
  }, [sectionRef, shouldDisable, enabled, spotlightRadius, glowColor]);
